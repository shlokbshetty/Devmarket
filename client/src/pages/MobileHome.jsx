    ...
    <div className="w-full bg-gray-50 dark:bg-[#0a0a0a] pb-24 text-gray-900 dark:text-white p-4 space-y-6 transition-colors">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Search apps, tools, and scripts..."
          className="w-full bg-white dark:bg-[#1a1a1a] rounded-2xl py-3 pl-11 pr-4 text-sm focus:outline-none focus:ring-1 focus:ring-[#1ed760] dark:focus:ring-[#1ed760] transition-all border border-gray-200 dark:border-[#222]" />
      </div>
      ...
      <section>
        <div className="flex items-end justify-between mb-4">
          ...
          <a href="#" className="text-[#1ed760] dark:text-[#1ed760] text-xs font-bold tracking-wider uppercase">View All</a>
        </div>
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-emerald-800 to-black p-5 flex flex-col justify-end min-h-[180px]">
          ...
          <div className="relative z-10 text-white">
            ...
            <button className="bg-[#1ed760] text-black font-bold py-2 px-5 rounded-full text-xs hover:bg-[#1ed760] transition-colors shadow-lg">Download Now</button>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white border border-gray-200 dark:border-transparent dark:bg-[#222] rounded-2xl p-4 relative group cursor-pointer hover:bg-gray-50 dark:hover:bg-[#2a2a2a] transition-colors shadow-sm dark:shadow-none">
          <div className="text-[#1ed760] dark:text-[#1ed760] mb-3">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4">
        {["All", "Games", "Productivity", "Utilities", "Design"].map((cat, i) => (
          <button key={cat} className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-colors ${i === 0 ? "bg-[#1ed760] dark:bg-[#1ed760] text-black border border-transparent" : "bg-white dark:bg-[#1a1a1a] text-gray-600 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-[#2a2a2a] border border-gray-200 dark:border-[#222]"}`}>
            {cat}
          </button>
        ))}
      </div>
            ...
            <div key={i} className="flex items-center gap-3 group cursor-pointer p-2 -mx-2 rounded-xl hover:bg-white dark:hover:bg-[#1a1a1a] transition-colors border border-transparent hover:border-gray-100 dark:hover:border-transparent hover:shadow-sm dark:hover:shadow-none">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${app.type === 'FREE' ? 'bg-gray-200 dark:bg-[#333] text-gray-700 dark:text-zinc-300' : 'bg-[#1ed760]/20 dark:bg-[#1ed760]/20 text-emerald-700 dark:text-[#1ed760]'}`}>{app.type}</span>
                  <div className="flex items-center gap-1 text-[10px] text-gray-500 dark:text-zinc-400 font-bold">
                    <Star className="text-[#1ed760] dark:text-[#1ed760] fill-[#1ed760] dark:fill-[#1ed760]" size={10} />{app.rating}
                  </div>
                </div>
              </div>
              <button className="w-8 h-8 rounded-full bg-white dark:bg-transparent border border-gray-200 dark:border-zinc-700 flex items-center justify-center text-gray-400 dark:text-zinc-400 group-hover:bg-[#1ed760] dark:group-hover:bg-[#1ed760] group-hover:text-black group-hover:border-[#1ed760] dark:group-hover:border-[#1ed760] transition-all shadow-sm dark:shadow-none">
                <Download size={14} />
              </button>
            </div>
          ))}
