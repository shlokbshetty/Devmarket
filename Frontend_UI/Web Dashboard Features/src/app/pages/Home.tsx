import { Search, ShoppingBag, ArrowRight, Download, Star } from "lucide-react";
  ...
  return (
    <div className="w-full bg-gray-50 dark:bg-[#0a0a0a] pb-24 text-gray-900 dark:text-white p-4 space-y-6 transition-colors">
      ...
      <div className="relative">
        ...
        <input 
          type="text" 
          placeholder="Search apps, tools, and scripts..." 
      ...
      <section>
        ...
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-emerald-800 to-black p-5 flex flex-col justify-end min-h-[180px]">
          ...
          <div className="relative z-10 text-white">
            ...
            <p className="text-gray-200 dark:text-zinc-300 text-xs mb-4 leading-relaxed max-w-[220px] drop-shadow-md">
              The visual IDE for creative developers.
            </p>
            <button className="bg-[#34d399] text-black font-bold py-2 px-5 rounded-full text-xs hover:bg-[#2ebc87] transition-colors shadow-lg">
              Download Now
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-[#59ff8b] rounded-2xl p-4 text-black relative flex flex-col justify-between cursor-pointer shadow-sm">
          <div>
            <h3 className="text-base font-bold mb-1">Bundle</h3>
            <p className="text-xs text-black/70 leading-tight mb-2">12 premium tools</p>
          </div>
          <div className="flex items-center justify-between mt-auto">
            <span className="text-xl font-black tracking-tight">$49</span>
            <ShoppingBag size={18} />
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4">
        {["All", "Games", "Productivity", "Utilities", "Design"].map((cat, i) => (
          <button 
            key={cat} 
            className={`whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold transition-colors ${i === 0 ? "bg-emerald-400 dark:bg-[#2ebc87] text-black border border-transparent" : "bg-white dark:bg-[#1a1a1a] text-gray-600 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-[#2a2a2a] border border-gray-200 dark:border-[#222]"}`}
          >
            {cat}
          </button>
        ))}
            ...
            <div key={i} className="flex items-center gap-3 group cursor-pointer p-2 -mx-2 rounded-xl hover:bg-white dark:hover:bg-[#1a1a1a] transition-colors border border-transparent hover:border-gray-100 dark:hover:border-transparent hover:shadow-sm dark:hover:shadow-none">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${app.type === 'FREE' ? 'bg-gray-200 dark:bg-[#333] text-gray-700 dark:text-zinc-300' : 'bg-emerald-100 dark:bg-[#2ebc87]/20 text-emerald-700 dark:text-[#2ebc87]'}`}>
                    {app.type}
                  </span>
                  <div className="flex items-center gap-1 text-[10px] text-gray-500 dark:text-zinc-400 font-bold">
                    <Star className="text-emerald-500 dark:text-[#2ebc87] fill-emerald-500 dark:fill-[#2ebc87]" size={10} />
                    {app.rating}
                  </div>
                </div>
              </div>
